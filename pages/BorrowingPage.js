...
class BorrowingPage {
  constructor(page) {
    ...
    // Return & Fine Modals/Buttons
    this.returnBtn  = page.locator('button, a').filter({ hasText: /Return|คืน/i });
    this.lostBtn    = page.locator('button, a').filter({ hasText: /Lost|สูญหาย/i });
    this.payBtn     = page.locator('button, a').filter({ hasText: /Pay|ชำระค่าปรับ/i });
    this.fineInput  = page.locator('input[name*="fine_amount"], #fine_amount, input[name*="amount"]');
