...
class MembersPage {
  ...
  async fillMemberInfo(code, name, email, phone, type = null) {
    ...
    if (code) await this.codeInput.fill(String(code));
    if (name) await this.nameInput.fill(String(name));
    if (email) await this.emailInput.fill(String(email));
    
    if (phone && await this.phoneInput.isVisible().catch(() => false))
    ...
    if (type && await this.typeSelect.isVisible().catch(() => false)) {
      ...
      const options = this.typeSelect.locator('option');
      const count = await options.count();
      for (let i = 0; i < count; i++) {
        const text = await options.nth(i).textContent();
        const val = await options.nth(i).getAttribute('value');
        if (text.toLowerCase().includes(type.toLowerCase()) || (val && val.toLowerCase().includes(type.toLowerCase()))) {
          await this.typeSelect.selectOption({ index: i });
          break;
        }
      }
    }
